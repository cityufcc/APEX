...
def make_lammps_Lat_param_T(conf, type_map, interaction, param, cal_setting=None):
    ...
    - Uses variables defined in `variable_Lat_param_T.in` for temperature and
    ...
    type_map_list = element_list(type_map)
    deepmd_version = param.get("deepmd_version", None)
    # Defaults (overridable via cal_setting)
    dump_step = 100
    tdamp = 100
    pdamp = 1000
    ...
    if ensemble == "adiabatic":
        ...
        # NPH + Langevin isothermal control
        ret += f"fix 1 all nph aniso 1.0 1.0 {pdamp} drag 1.0\n"
        ret += f"fix 5 all langevin ${{temperature}} ${{temperature}} {tdamp} {int(velocity_seed)}\n"
    else:
        # Default Nose-Hoover NPT (anisotropic barostat)
        ret += (
